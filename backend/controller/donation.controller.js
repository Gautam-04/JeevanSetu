...
const verifyPayment = async(req,res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Payment verified successfully', newDonation });
    } catch (error) {
        console.log(error);
        res.status(500).json({ success: false, message: 'Server Error' });
    }
...
const getDonations = async(req,res) => {
    ...
    try {
        ...
        const donations = await Fundraiser.find(query).populate('donations').sort({ createdAt: -1 });
...
const createFundraiser = async(req,res) => {
    ...
    try {
        ...
        if(!fundraiser){
            return res.status(500).json({success: false ,message: "Error creating fundraiser"});
        }

        return res.status(201).json({success: true ,message: "Fundraiser created successfully",fundraiser});
    } catch (error) {
        console.log(error);
        return res.status(500).json({success: false ,message: "Internal server error"});
    }
...
const deleteFundraiser = async(req,res) => {
    ...
    try {
        ...
        return res.status(200).json({success: true ,message: "Fundraiser and associated donations deleted successfully"});
    } catch (error) {
        console.log(error);
        return res.status(500).json({success: false ,message: "Internal server error"});
    }
...
export {createFundraiser,deleteFundraiser,getFundraisers,createOrder,verifyPayment,getDonations}
